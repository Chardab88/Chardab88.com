...
const STORAGE_KEY = "Nasgrid_easy_stats";
const DAILY_KEY = "Nasgrid_easy_daily";
...
fetch("easy_drivers.json")
  .then(res => res.json())
  .then(data => {
      easydrivers = data;

      easycleanedDrivers = easydrivers.map(d =>
          d.name.toLowerCase().replace(/\s/g, "")
      );
...
function startRound() {
    ...
    driver = getDailyDriver(easydrivers);
    driverClean = driver.name.toLowerCase().replace(/\s/g, "");
    currentGuess = 0;
    dailyGuesses = [];
...
function applyColors(index, guessedDriver) {
    ...
    if (guessedDriver.team === driver.team) {
        team[index].parentElement.style.backgroundColor = "green";
    }else if (driver.formteam.includes(guessedDriver.tean)){
...
function restoreBoard() {
    ...
    if (wonToday) {
        ...
        input.placeholder = "Out of guesses! Come back tomorrow.";
    }
}
    if (currentGuess >= 6) {
        input.disabled = true;
